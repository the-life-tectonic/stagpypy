...
class SLURM(Job):
    ...
    def __str__(self):
        return super(SLURM,self).__str__('slurm')
...
def get_job(system,name,walltime,cpus,account,mpi_command,queue_name=None,**kwargs):
# Select the correct sized queue from the system
    if queue_name==None:
        q=system.get_queue(cpus,walltime)
    else:
        queues=[q for q in system.queues if q.name==queue_name]
        if len(queues)==0:
            raise ValueError("No such queue '%s'"%queue_name)
        q=queues[0]
